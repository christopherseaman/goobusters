...
import logging
import sys
from src.multi_frame_tracking.multi_frame_tracker import MultiFrameTracker, process_video_with_multi_frame_tracking
...
#MULTI_FRAME_DIR = os.path.join(OUTPUT_DIR, "multi_frame_output")  --> creates subfolder for the multi-frame script outputs, if we want 
...
def delete_existing_annotations(client, study_uid, series_uid, label_id, group_id=None):
    ...
    Adapts to available methods in the MD.ai client.
    """
    try:
        print(f"\nChecking for existing annotations for series {series_uid}...")
        
        # Get existing annotations using available methods
        existing_annotations = []
        
        # Try different approaches based on available methods
        if hasattr(client, 'search_annotations'):
            # Using search_annotations
            filter_criteria = {
            ...
            print("Used search_annotations method")
            
        elif hasattr(client, 'get_annotations'):
            # Using get_annotations
            query = {
                'StudyInstanceUID': study_uid,
                'SeriesInstanceUID': series_uid,
                'labelId': label_id
            ...
            print("Used get_annotations method")
            
        elif hasattr(client, 'list_annotations'):
            # Using list_annotations and filter
            all_annotations = client.list_annotations(
                project_id=PROJECT_ID,
                dataset_id=DATASET_ID
            )
            ...
            existing_annotations = [
                ...
                ann.get('labelId') == label_id and
                (group_id is None or ann.get('groupId') == group_id)
            ]
            print("Used list_annotations method with filtering")
        ...
        # Delete annotations using available methods
        delete_method = None
        if hasattr(client, 'delete_annotation'):
            delete_method = client.delete_annotation
        elif hasattr(client, 'remove_annotation'):
            delete_method = client.remove_annotation
        
        if not delete_method:
            print("No suitable method found to delete annotations.")
            return 0
        
        # Delete annotations one by one
        deleted_count = 0
        for ann in existing_annotations:
            try:
                annotation_id = ann.get('id')
                if annotation_id:
                    delete_method(annotation_id)
                    deleted_count += 1
            except Exception as e:
...
def upload_masks_to_mdai(client, masks_data, project_id, dataset_id=None):
    ...
    try:
        ...
        # Delete existing annotations to avoid duplicates
        for study_uid, series_uid in cleanup_pairs:
...
def test_mdai_upload(study_uid, series_uid, frame_number, mask_path=None):
    """
    Test MD.ai upload with either a generated mask or a mask loaded from file
    
    Args:
        study_uid: The StudyInstanceUID
        series_uid: The SeriesInstanceUID
        frame_number: The frame number
        mask_path: Optional path to a mask image file
    """
    print(f"\n==== TESTING MD.AI UPLOAD ====")
    ...
    if mask_path and os.path.exists(mask_path):
        ...
        print(f"Creating test C-shaped mask")
        mask = np.zeros((512, 512), dtype=np.uint8)
        cv2.ellipse(mask, (256, 256), (150, 100), 0, 45, 315, 1, thickness=-1)
    
    # Print mask stats
    print(f"Mask shape: {mask.shape}")
    print(f"Mask sum: {np.sum(mask)} non-zero pixels")
    
    # Save mask visualization for reference
    debug_dir = os.path.join(os.getcwd(), "debug_mdai_test")
    os.makedirs(debug_dir, exist_ok=True)
    viz_path = os.path.join(debug_dir, "test_mask.png")
    cv2.imwrite(viz_path, mask * 255)
    print(f"Saved mask visualization to: {viz_path}")
    
    # Prepare mask data
    mask_data = prepare_mask_data(mask, frame_number, study_uid, series_uid)
    if not mask_data:
        print("Error: Failed to prepare mask data")
        return False
    
    # Upload to MD.ai
    try:
        print(f"\nUploading mask to MD.ai...")
        upload_result = upload_masks_to_mdai(
            mdai_client, 
            [mask_data], 
            PROJECT_ID,
            DATASET_ID
        ...
        if upload_result:
            print(f"✅ Success! Mask uploaded to MD.ai")
            print(f"Exam number: {find_exam_number(study_uid, annotations_json)}")
            return True
        else:
            print(f"❌ Failed to upload mask to MD.ai")
            return False
    except Exception as e:
...
if __name__ == "__main__":
    # Example study/series from your dataset
    test_study = "1.2.826.0.1.3680043.8.498.12762211632497404572246503032980657292"
    test_series = "1.2.826.0.1.3680043.8.498.90262783102403545676047413537747709850"
    test_frame = 53  # Frame number to test
    
    # Optional - path to existing mask file
    # test_mask = "path/to/mask.png"  # Set to None to generate a C-shaped mask
    
    # Run test
    test_result = test_mdai_upload(test_study, test_series, test_frame)
        ...
        for index, annotation in tqdm(issue_annotations.iterrows(), 
            ...
            try:
                ...
                result = process_video_with_multi_frame_tracking(
                   ...
                   annotations_df=pd.DataFrame([annotation]),  # Convert single annotation to DataFrame
                   study_uid=study_uid,
                   series_uid=series_uid,
                   ...
                   project_id=PROJECT_ID,
                   dataset_id=DATASET_ID,
                   upload_to_mdai=True  # Set this according to your needs
)
