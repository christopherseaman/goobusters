...
from metrics import calculate_jaccard_index, apply_binary_threshold  
...
# Initialise OpticalFlowProcessor for PWC-Net and Farneback methods
...
def debug_visualize(frame, mask, flow, frame_number, gt_mask=None, threshold=0.5):
    """Visualize optical flow and calculate the Jaccard Index if ground-truth is available."""
    h, w = frame.shape[:2]
    ...
    # Apply binary threshold and calculate Jaccard Index if gt_mask is provided
    ...
    if gt_mask is not None:
        jaccard_index = calculate_jaccard_index(binary_pred_mask, gt_mask)
        cv2.putText(grid, f"Jaccard Index: {jaccard_index:.2f}", (10, h+30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
...
def track_and_save_masks_as_video(annotation, output_dir, pwc_processor, farneback_processor):
    ...
    # Process frames and calculate Jaccard index with gt_mask
...
def save_combined_video(video_path, output_video_path, gt_mask, frame_number, debug_dir, pwc_processor, farneback_processor):
    ...
    for i in range(total_frames):
        ...
        if i == 0:
            ...
            jaccard_index = calculate_jaccard_index(binary_pred_mask, gt_mask)
            print(f"Frame {i} - Jaccard Index: {jaccard_index:.2f}")
